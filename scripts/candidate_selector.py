...
def _extract_version(text: str) -> Optional[str]:
    """テキストから semver 形式のバージョン番号（v1.2.3）を抽出する。"""
...
def select_and_summarize(articles: list[dict], config: dict) -> list[dict]:
    ...
    for article in priority_articles:
        ...
        # タイトル → URL の順でバージョン番号を抽出
        article["version"] = (
            _extract_version(article.get("title", ""))
            or _extract_version(article.get("url", ""))
        )
