...
// /news ニュース未取得: opening×reaction×body×emoji×tail = 15×10×12×8×10 = 144,000通り
const _MSG_NO_NEWS = {
...
// /news 成功時末尾: prefix×body×suffix×emoji = 6×20×12×10 = 14,400通り (inside _..._)
const _MSG_NEWS_TAIL = {
  prefix:  ["", "ちなみに、", "そういえば、", "最後に、", "おまけ：", "P.S. "],
...
// /ask 質問未入力: opening×reaction×body×hint×example = 12×10×10×8×6 = 57,600通り
const _MSG_ASK_NOINPUT = {
  opening:  ["えっと、", "ん？", "あれ、", "むむ、", "待って、", "質問が…", "（質問がないよ！）", "あのー、",
             "おっと！", "うっ…", "ちょっと！", "んん？"],
  reaction: ["", " （なにを聞きたいの？）", " （空っぽだよ？）", " （えっ、書き忘れ？）",
...
// /ask Gemini未設定: opening×reaction×body×emoji×request = 10×8×12×8×6 = 46,080通り
const _MSG_ASK_NO_API = {
  opening:  ["ごめんね、", "うぅ、", "あのね、", "むむっ、", "ちょっと待って、", "えっとね、",
             "残念なんだけど、", "実はね、", "申し訳ないんだけど、", "今はね、"],
  reaction: ["", " （ぐすん）", " （頑張りたいのに…）", " （ごめんなさい）",
...
// /ask 応答末尾: opening×powered×comment×emoji = 10×15×8×6 = 7,200通り (inside _..._)
const _MSG_ASK_POWERED = {
  opening:  ["", "ちなみに、", "今回は、", "今日は、", "さてさて、", "ね！ ", "ふふ、", "えへ、", "あのね、", "ところで、"],
...
// 未知コマンド: opening×reaction×body×suggest×emoji = 12×10×10×8×6 = 57,600通り
const _MSG_UNKNOWN_CMD = {
  opening:  ["えっ？", "ん？", "（どれだろ…）", "あれ、", "むむ？", "わからないよ〜、", "うーん、", "ぽかん…",
             "はて？", "んんん？", "それは…", "うぅ？"],
  reaction: ["", " （知らない！）", " （そのコマンドは…）", " （えっ、何それ？）", " （どこで覚えたの？）",
...
function pickMsg(type) {
  if (type === "no_news") {
    return _r(_MSG_NO_NEWS.opening) + _r(_MSG_NO_NEWS.reaction) + _r(_MSG_NO_NEWS.body)
         + _r(_MSG_NO_NEWS.emoji) + " " + _r(_MSG_NO_NEWS.tail);
  }
  if (type === "news_intro") {
    return _r(_MSG_NEWS_INTRO.opening) + _r(_MSG_NEWS_INTRO.vibe)
         + _r(_MSG_NEWS_INTRO.body) + _r(_MSG_NEWS_INTRO.hype);
  }
  if (type === "news_tail") {
    return `_${_r(_MSG_NEWS_TAIL.prefix)}${_r(_MSG_NEWS_TAIL.body)}${_r(_MSG_NEWS_TAIL.suffix)}${_r(_MSG_NEWS_TAIL.emoji)}_`;
  }
  if (type === "ask_noinput") {
    return _r(_MSG_ASK_NOINPUT.opening) + _r(_MSG_ASK_NOINPUT.reaction)
         + _r(_MSG_ASK_NOINPUT.body) + _r(_MSG_ASK_NOINPUT.hint) + _r(_MSG_ASK_NOINPUT.example);
  }
  if (type === "ask_no_api") {
    return _r(_MSG_ASK_NO_API.opening) + _r(_MSG_ASK_NO_API.reaction)
         + _r(_MSG_ASK_NO_API.body) + "\n"
         + _r(_MSG_ASK_NO_API.emoji) + " " + _r(_MSG_ASK_NO_API.request);
  }
  if (type === "ask_powered") {
    return `_${_r(_MSG_ASK_POWERED.opening)}${_r(_MSG_ASK_POWERED.powered)}${_r(_MSG_ASK_POWERED.comment)}${_r(_MSG_ASK_POWERED.emoji)}_`;
  }
  if (type === "rate_limit") {
    return _r(_MSG_RATE_LIMIT.opening) + _r(_MSG_RATE_LIMIT.reaction)
         + _r(_MSG_RATE_LIMIT.body) + _r(_MSG_RATE_LIMIT.emoji) + _r(_MSG_RATE_LIMIT.tail);
  }
  // unknown_cmd
  return _r(_MSG_UNKNOWN_CMD.opening) + _r(_MSG_UNKNOWN_CMD.reaction)
       + _r(_MSG_UNKNOWN_CMD.body) + " " + _r(_MSG_UNKNOWN_CMD.suggest) + " " + _r(_MSG_UNKNOWN_CMD.emoji);
}
