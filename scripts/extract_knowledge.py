...
URLのパスや記事タイトルのキーワードを最大限活用してください。"""
    ...
    try:
        ...
        if resp.status_code == 429:
            logger.info("レート制限。300秒待機後にリトライ...")
            time.sleep(300)
            # 1回だけリトライ
            try:
                resp2 = requests.post(url_endpoint, json=payload, timeout=30)
                resp2.raise_for_status()
                data2 = resp2.json()
                raw_text2 = data2["candidates"][0]["content"]["parts"][0]["text"].strip()
                return json.loads(raw_text2)
            except Exception:
                logger.error("リトライも失敗。次回実行時に再試行します。")
                return {"_rate_limited": True}  # 429専用マーカー
        return None
...
def run(force: bool = False, dry_run: bool = False, limit: int = 0):
    ...
    MAX_CONSECUTIVE_RATE_LIMITS = 3  # 連続3回429で早期終了
    ...
    for i, article in enumerate(pending, 1):
        ...
        if knowledge is None:
            logger.warning(f"  ❌ 抽出失敗（永続エラー）: {url}")
            error_count += 1
            processed_ids.add(article_id)  # 永続エラーは次回もスキップ
            consecutive_rate_limits = 0
            continue

        if knowledge.get("_rate_limited"):
            logger.warning(f"  ⏳ レート制限エラー（次回リトライ）: {url}")
            error_count += 1
            consecutive_rate_limits += 1
            # 連続レート制限でAPIクォータ枯渇と判断し早期終了
            if consecutive_rate_limits >= MAX_CONSECUTIVE_RATE_LIMITS:
                logger.warning(
                    f"⚠️ 連続{MAX_CONSECUTIVE_RATE_LIMITS}回のレート制限エラー。"
                    "APIクォータ枯渇と判断し、残り記事は次回実行に延期します。"
                )
                break
            # processed_ids に追加しない → 次回実行時に再試行される
            continue

        # API成功 → 連続レート制限カウンターをリセット
        consecutive_rate_limits = 0
